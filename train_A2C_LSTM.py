...
learning_rate = 0.005 # 네트워크의 learning rate
        
run_step = 100000 if train_mode else 0 # 학습 모드에서 진행할 스텝 수 
...
save_path = f"C:/AllProgramming/MyProjects/UnityProjects/RL_TheHardestGame/saved_models/TheHardestGame/A2C/{date_time}"
if os.path.exists(save_path) == False:
    os.makedirs(save_path)
load_path = f"C:/AllProgramming/MyProjects/UnityProjects/RL_TheHardestGame/saved_models/TheHardestGame/A2C/20241208050446"
torch.cuda.init()
...
class A2C(nn.Module):
    def __init__(self, input_shape=(3, 128, 256), lstm_hidden_size=64, lstm_num_layers=1):
        ...
        self.fc_common = nn.Linear(lstm_hidden_size, 32)  # 공통 피처 추출
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        time_steps = C // 3
        assert C % 3 == 0, "Input channels must be divisible by 3."
        
        x = x.view(B, time_steps, 3, H, W)
