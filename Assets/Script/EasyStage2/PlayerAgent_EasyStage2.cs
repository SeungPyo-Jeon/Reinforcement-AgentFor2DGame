...
public class PlayerAgent_EasyStage2 : Agent
{
    // Start is called once before the first execution of Update after the MonoBehaviour is created
    Rigidbody2D m_RigidBody;
    Transform m_Transform;
    StageManager_EasyStage2 StageManager;
    ...
    public override void Initialize()
    {
        ...
        StageManager = GameObject.Find("StageManager").GetComponent<StageManager_EasyStage2>();
    }
    public void Start()
    {
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(m_Transform.position.x);
        sensor.AddObservation(m_Transform.position.y);
        sensor.AddObservation(StageManager.SafetyZone.transform.position.x);
        sensor.AddObservation(StageManager.SafetyZone.transform.position.y);
    }
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        Move(dir);
        SetReward(StageManager.getDistance(1.0f, 1.0f));
    }
    ...
    {
        // m_IsSafe = true;
        float reward = StageManager.EnterSafetyZone_Agent();
        if (reward > -1.0f)
        {
