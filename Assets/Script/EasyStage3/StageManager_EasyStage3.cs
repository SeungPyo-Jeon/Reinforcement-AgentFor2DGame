...
public class StageManager_EasyStage3 : MonoBehaviour
{
    [SerializeField]
    ...
    void Start()
    {
        ...
        lineList = GameObject.FindGameObjectsWithTag("Line").ToList();
    ...
    public void Reset()
    {
        ...
        if (resetMode == "EaseStage3")
        {
            Player.transform.position = Player.GetComponent<PlayerAgent_EasyStage3>().initPos;  //new Vector3(UnityEngine.Random.Range(1, 20), UnityEngine.Random.Range(1, 12), 0);
            /*do
            {
        ...
        {
            Player.transform.position = Player.gameObject.GetComponent<PlayerAgent_EasyStage3>().initPos;
        }
        distance = Vector2.Distance(Player.transform.position, SafetyZone.transform.position);
    }
