...
learning_rate = 0.001 # 네트워크의 learning rate
...
save_path = f"C:/AllProgramming/MyProjects/UnityProjects/RL_TheHardestGame/saved_models/TheHardestGame/A2C_EasyStage3/{date_time}"
if os.path.exists(save_path) == False:
    os.makedirs(save_path)
load_path = f"C:/AllProgramming/MyProjects/UnityProjects/RL_TheHardestGame/saved_models/TheHardestGame/A2C_EasyStage3/20241207234744"

device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # GPU 사용 가능 여부에 따라 device 설정

game = r"C:\AllProgramming\MyProjects\UnityProjects\RL_TheHardestGame\BUILD_EasyStage3_Visual_Position"
...
class A2C( torch.nn.Module):
    ...
    def forward( self, x):
        ...
        x = self.dropout2(x)
        x = F.relu( self.d3(x))
        pi1 = F.softmax(self.pi1(x),dim=0)
        pi2 = F.softmax(self.pi2(x),dim=0)
        v = self.v(x)
        return pi1, pi2, v
...
class A2CAgent:
    ...
    def train_model(self, state, action, reward, next_state, done):
        ...
        loss = critic_loss + actor_loss1*10 + actor_loss2*10
...
if __name__ == '__main__':
    ...
    env = UnityEnvironment(file_name = game, side_channels=[engine_configuration_channel], no_graphics=True) # 유니티 환경, side_channel은 해상도, timesclae, graphic quality 등을 설정할 때 사용
    env.reset()
    ...
    for step in tqdm(range(run_step + test_step)):
        ...
        state = state[:,:,0]# cv2.cvtColor(state, cv2.COLOR_BGR2GRAY)
        #cv2.imshow('state', state) 
        position = dec.obs[1][:,:]
        agent_pos = bilinear_interpolation_to_array( position[0,-4], a_max[1]-position[0,-3])
        target_pos = bilinear_interpolation_to_array( position[0,-2], a_max[1]-position[0,-1])
        #cv2.imshow('agent_pos', agent_pos)
        #cv2.imshow('target_pos', target_pos)
        #cv2.waitKey(0)
        #time.sleep(1)
        #cv2.destroyAllWindows()
        state = np.stack([state, agent_pos, target_pos], axis=0)
        ...
        next_state = dec.obs[OBS][0,-3:,:,:]

        if next_state.shape[0] == 0:
            next_state = np.ones_like((128,256))
            poxiton = np.ones_like(position)
        else:
            next_state = np.transpose(next_state[:,:,:],(1,2,0))
            next_state = next_state[:,:,0]#cv2.cvtColor(np.transpose(next_state[:,:,:],(1,2,0)), cv2.COLOR_BGR2GRAY)
            position = dec.obs[1][:,:]

        agent_pos = bilinear_interpolation_to_array( position[0,-4],a_max[1]-position[0,-3])
        target_pos = bilinear_interpolation_to_array( position[0,-2], a_max[1]-position[0,-1])
        next_state = np.stack([next_state, agent_pos, target_pos], axis=0)
        score += reward[0]
        
        if train_mode: 
            loss, actor1_loss, actor2_loss = agent.train_model(state, action, [reward[0]], next_state, [done])
