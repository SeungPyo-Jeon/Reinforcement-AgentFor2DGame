...
public class StageManager_EasyStage1 : MonoBehaviour
{
    [SerializeField]
    ...
    public void Reset()
    {
        ...
        {
            Player.transform.position = new Vector3(UnityEngine.Random.Range(6, 15), UnityEngine.Random.Range(4, 8), 0);
            do
            {
                SafetyZone.transform.position = new Vector3(UnityEngine.Random.Range(6, 15), UnityEngine.Random.Range(4, 8), 0);
            } while (Vector2.Distance(Player.transform.position, SafetyZone.transform.position) < 3);

        }
        else
        {
            Player.transform.position = Player.gameObject.GetComponent<PlayerAgent_EasyStage1>().initPos;
        }
        distance = Vector2.Distance(Player.transform.position, SafetyZone.transform.position);
    }
